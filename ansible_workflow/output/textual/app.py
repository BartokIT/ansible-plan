...
from textual.widgets import Button, DataTable, Footer, Header, RichLog, Tree, Static

from ...api_client import ApiClient
from ...workflow import NodeStatus, WorkflowStatus
...
class TextualWorkflowOutput(WorkflowOutput):
    ...
    class WorkflowApp(App):
        ...
        def on_mount(self) -> None:
            ...
            self.set_interval(1, self.check_workflow_completion)

        @work(thread=True)
        def check_workflow_completion(self):
            status = self.api_client.get_workflow_status()
            if status in [WorkflowStatus.ENDED.value, WorkflowStatus.FAILED.value]:
                self.exit()
